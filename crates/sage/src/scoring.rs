...
impl<'db> Scorer<'db> {
    ...
    fn matched_peaks_with_isotope(
        ...
        for peak_mass in &query.masses {
            for charge in 1..max_fragment_charge {
                for frag in candidates.page_search(*peak_mass, charge) {
    ...
    fn remove_matched_peaks(&self, query: &mut ProcessedSpectrum, psm: &Feature) {
        ...
        query.mobilities = match query.mobilities {
            Some(ref mut mobilities) => Some(mobilities
                .drain(..)
                .enumerate()
                .filter(|(idx, _)| !to_remove_idx.contains(idx))
                .map(|(_, mobility)| mobility)
                .collect()),
            None => None,
        };
            ...
            for charge in 1..max_fragment_charge {
                ...
                if let Some(peak_idx) = crate::spectrum::select_most_intense_peak(
                    ...
                    score.ppm_difference +=
                        query.intensities[peak_idx] * (mz - query.masses[peak_idx]).abs() * 2E6 / (mz + query.masses[peak_idx]);
                    let peak_intensity = query.intensities[peak_idx];

                    let exp_mz = query.masses[peak_idx] + PROTON;
