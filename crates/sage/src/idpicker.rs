...
fn group_node_with_same_edge<'a>(l1 : &'a Vec<String>, l2: &'a Vec<String>) -> Vec<(Vec<&'a str>, &'a str)> {
    ...
    let groups: HashMap<&str, &Vec<&str>> = reverse_map.values()
...
fn group_node_with_protein<'a>(l2 : &'a Vec<Vec<&str>>, l1: &'a Vec<&str>) ->  Vec<(&'a Vec<&'a str>, Vec<&'a str>)>{
...
fn separate_into_clusters<'a>(data: Vec<(&'a Vec<&'a str>, Vec<&'a str>)>) -> Vec<Vec<(Vec<&'a str>, Vec<&'a str>)>> {
    ...
    while used_keys.len() < node_dict.len() {
        let root = node_dict.keys().find(|k| !used_keys.contains(*k)).unwrap().clone();
        ...
        loop {
            ...
            let keys: Vec<_> = node_dict.keys()
                .filter(|key| !cluster.contains(*key) && !node_dict[*key].is_disjoint(&proteins)).collect();
...
fn reduce_cluster<'a>(data: Vec<(Vec<&'a str>, Vec<&'a str>)>) -> Vec<(&'a str, String)> {
    ...
    while used_peptides.len() < total_peptides.len() {
        ...
        data.iter().filter(|(peptides, _)| !used_peptides.contains(peptides))
            .for_each(|(peptides, proteins)| {
                prot_dict.entry(proteins.clone()).or_default().insert(peptides.clone());
            });

        // Compute scores for sorting
        let mut score_dict: BTreeMap<Vec<&str>, (usize, usize)> = prot_dict.iter()
            .map(|(key, peptides)| {
                (key.clone(), (peptides.len(), *overall_score_dict.get(key).unwrap_or(&0)))
            })
            .collect();

        // Sort proteins based on (peptide count, overall score)
        let sorted_keys = score_dict.into_iter()
...
fn group_node() {
   ...
   /* let data2 = group_by_proteins(data1);
